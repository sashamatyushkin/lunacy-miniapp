...
#
# Бесплатные туннели на этой сети нестабильны: localtunnel не всегда выдаёт
# запрошенный поддомен, tunnelmole меняет адрес на реконнекте. Вместо борьбы за
# фиксированный URL супервизор принимает ЛЮБОЙ текущий адрес и при каждом его
# изменении обновляет две вещи:
...
while true; do
  ...
  : > "$TUN_LOG"
  # запрашиваем фиксированный поддомен; сервер может выдать другой — примем любой
  nohup npx -y localtunnel --port "$PORT" --subdomain "$SUB" >> "$TUN_LOG" 2>&1 &
  LT_PID=$!
