...
Mini App и API живут в разных местах, и это не случайность.

**Фронтенд — GitHub Pages.** Бесплатные туннели показывают браузеру страницу-предупреждение («Please proceed with caution», «You are about to visit…»), и Telegram открывает в Mini App именно её. Обойти это заголовком нельзя: первую загрузку HTML делает сам WebView. Pages отдаёт статику по стабильному HTTPS без заглушек, поэтому адрес Mini App вводится в BotFather один раз и больше не меняется.

**API — туннель с этого Mac.** Здесь заглушка не мешает: все запросы идут из уже загруженного приложения, и фронтенд добавляет к ним `bypass-tunnel-reminder`, а Telegram ходит на вебхук не браузером.

**Адрес API не вшит в сборку.** Он лежит в `api-endpoint.json` рядом со статикой и читается один раз при старте. Поэтому переподнятый туннель не требует пересборки — `npm run up` сам обновляет этот файл (`npm run endpoint` делает то же вручную).

### Туннель

`scripts/tunnel.sh` перебирает провайдеров по очереди, потому что на разных сетях живы разные:

1. `cloudflared` — лучший, но его edge-адреса часто режет DPI или VPN. В `/tmp/lunacy-tunnel.log` это видно как `failed to dial to edge` (UDP :7844) или `TLS handshake with edge error: EOF` (TCP 443).
2. `tunnelmole` — работает без аккаунта и не показывает заглушку.
3. `localtunnel` — последний резерв, капризен и с заглушкой.
...
Собирает фронтенд с `base=/lunacy-miniapp/`, кладёт `404.html` (у Pages нет SPA-фолбэка) и пушит статику в ветку `gh-pages`.
